  ...
  useEffect(() => {
    if (supplierId) {
      filteredMutation.mutate(
        { supplier_id: supplierId, status: 'Re-Transact' },
        {
          onSuccess: data => {
      ...
      },
      {
        onSuccess: (serverData, variables) => {
          navigation.navigate(ScreenNames.HOME_STACK.TRANSACTION_DETAIL, {
            ...
            prevRouteName: 'RetransactTransactionsScreen',
          });
        },
      },
    );
  ...
  const renderItem = ({ item }: { item: TransactionItem }) => (
    <TouchableOpacity
      ...
      <View style={styles.badge}>
        <Text style={styles.badgeText}>Re-Transact</Text>
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Re-Transact Transactions</Text>
      </View>
      ...
      {!filteredMutation.isPending && transactions.length === 0 && (
        <View style={styles.emptyContainer}>
          <MaterialIcons name="refresh" size={48} color="#D0D0D0" />
          <Text style={styles.emptyText}>No re-transact transactions</Text>
        </View>
      )}
...
export default RetransactTransactions;
